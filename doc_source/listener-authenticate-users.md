...
+ **Single page applications with Java scripts that load every few seconds**—By default, after the authentication session cookie expires, the AJAX calls are redirected to the IdP and are blocked\. If you use the `deny` option, the load balancer returns an HTTP 401 Unauthorized error to these AJAX calls\.
